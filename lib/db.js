...
export async function unmatchedDocumentsWithAmount() {
  return await sql`
    select id, doc_date, amount, vendor, attachment_name
    from documents
...
// Alle boekingen met statusindicatie + gekoppeld document (voor het totaaloverzicht).
export async function allTransactions() {
  return await sql`
    select t.id, t.tx_date, t.amount, t.counterparty, t.description, t.receipt_status,
           t.matched_doc_id, d.match_diff, d.attachment_name as doc_name
    from transactions t
...
// Handmatig een document aan een transactie koppelen; verschil (tx - bon) wegschrijven.
export async function linkManual(docId, txId) {
  const d = await sql`select amount from documents where id = ${docId}`;
  const t = await sql`select amount from transactions where id = ${txId}`;
  if (!d[0] || !t[0]) return null;
  const diff = Math.round((Math.abs(Number(t[0].amount)) - Number(d[0].amount)) * 100) / 100;
  await sql`update transactions set matched_doc_id = ${docId} where id = ${txId}`;
  await sql`update documents set matched_tx_id = ${String(txId)}, match_diff = ${diff} where id = ${docId}`;
  return { diff };
}

// Koppeling losmaken vanaf de bon-kant.
export async function unlinkByDoc(docId) {
  const d = await sql`select matched_tx_id from documents where id = ${docId}`;
  if (d[0]?.matched_tx_id) {
    await sql`update transactions set matched_doc_id = null where id = ${Number(d[0].matched_tx_id)}`;
  }
  await sql`update documents set matched_tx_id = null, match_diff = null where id = ${docId}`;
}

// Koppeling losmaken vanaf de transactie-kant.
export async function unlinkByTx(txId) {
  const t = await sql`select matched_doc_id from transactions where id = ${txId}`;
  if (t[0]?.matched_doc_id) {
    await sql`update documents set matched_tx_id = null, match_diff = null where id = ${t[0].matched_doc_id}`;
  }
  await sql`update transactions set matched_doc_id = null where id = ${txId}`;
}

// Een boeking verwijderen (koppeling eerst losmaken zodat de bon niet dangelt).
export async function deleteTransaction(txId) {
  await unlinkByTx(txId);
  await sql`delete from transactions where id = ${txId}`;
}

// Een bon/document verwijderen (koppeling eerst losmaken).
export async function deleteDocument(docId) {
  await unlinkByDoc(docId);
...
export async function vatByMonth() {
  const rows = await sql`
    select t.id, t.tx_date, t.amount, t.counterparty, t.description,
           t.receipt_status, t.matched_doc_id, d.vat as doc_vat
    from transactions t
    left join documents d on d.id = t.matched_doc_id
    where t.amount <> 0
    order by t.tx_date desc, t.id desc`;
  ...
  // Neon levert date-kolommen als Date-object; normaliseer naar 'YYYY-MM-DD'.
  ...
  for (const r of rows) {
    ...
    let vat = 0;
    let estimated = false;
    let source = 'geen'; // 'bon' | 'schatting' | 'geen'
    ...
    const iso = isoDate(r.tx_date); // 'YYYY-MM-DD'
    const ym = iso.slice(0, 7); // 'YYYY-MM'
    const q = `${ym.slice(0, 4)}-Q${Math.ceil(Number(ym.slice(5, 7)) / 3)}`;
    const dir = amt > 0 ? 'output' : 'input'; // verkoop / inkoop

    const mo = months.get(ym) || { month: ym, output: 0, input: 0, estimated: 0, rows: [] };
    if (dir === 'output') mo.output = r2(mo.output + vat);
    else mo.input = r2(mo.input + vat);
    if (estimated) mo.estimated = r2(mo.estimated + vat);
    mo.rows.push({
      id: r.id, tx_date: iso, amount: amt,
      counterparty: r.counterparty, description: r.description,
      vat, direction: dir, source,
    });
    months.set(ym, mo);

    const qa = quarters.get(q) || { quarter: q, output: 0, input: 0, estimated: 0 };
    if (dir === 'output') qa.output = r2(qa.output + vat);
    else qa.input = r2(qa.input + vat);
    if (estimated) qa.estimated = r2(qa.estimated + vat);
    quarters.set(q, qa);
  }

  const monthsArr = [...months.values()]
    .map((m) => ({ ...m, reserve: r2(m.output - m.input) }))
    .sort((a, b) => (a.month < b.month ? 1 : -1));
  const quartersArr = [...quarters.values()]
    .map((q) => ({ ...q, reserve: r2(q.output - q.input) }))
    .sort((a, b) => (a.quarter < b.quarter ? 1 : -1));

  return { months: monthsArr, quarters: quartersArr };
}
