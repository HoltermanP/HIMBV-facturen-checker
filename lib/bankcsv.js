...
export function parseBankCsv(text) {
  ...
  // Heeft de eerste regel herkenbare koppen? Dan kolom-op-naam parsen.
  if (idx.date >= 0 && idx.amount >= 0) {
    return parseWithHeader(rows, idx, warnings, header);
  }

  // Geen koppen herkend -> probeer het kolom-vaste ABN AMRO TAB-formaat.
  if (looksLikeAbnTab(rows[0])) {
    return parseAbnTab(rows, warnings);
  }

  throw new Error(`Kon datum/bedrag-kolom niet herkennen. Gevonden kolommen: ${header.join(', ')}`);
}

// --- Parser 1: CSV mét kopregel -------------------------------------------

function parseWithHeader(rows, idx, warnings, header) {
  const seen = new Map();
  const out = [];
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    out.push(
      makeRow(seen, {
        tx_date,
        amount,
        counterparty: idx.name >= 0 ? clean(get(row, idx.name)) : null,
        description: idx.desc >= 0 ? clean(get(row, idx.desc)) : null,
      }),
    );
  }
...
// --- Parser 2: ABN AMRO TAB (kolom-vast, geen koppen) ---------------------

// Bekende MT940-tags in het omschrijvingsveld (/NAME/, /REMI/ enz.).
const ABN_TAGS = ['TRTP', 'IBAN', 'BIC', 'NAME', 'REMI', 'EREF', 'MARF', 'CSID', 'ORDP', 'BENM', 'PREF', 'SVWZ', 'RTRN', 'ID'];

// Herkenning: kol 2 is een datum YYYYMMDD en kol 6 is een bedrag.
function looksLikeAbnTab(row) {
  return row.length >= 7 && /^\d{8}$/.test((row[2] || '').trim()) && !Number.isNaN(parseDutchNumber(row[6]));
}

function parseAbnTab(rows, warnings) {
  const seen = new Map();
  const out = [];

  for (let i = 0; i < rows.length; i++) {
    const row = rows[i];
    if (!looksLikeAbnTab(row)) {
      if (row.some((c) => c.trim() !== '')) warnings.push(`Regel ${i + 1} overgeslagen (geen ABN-transactie).`);
      continue;
    }

    const tx_date = normalizeDate(get(row, 2)); // boekdatum
    const amount = parseDutchNumber(get(row, 6)); // bedrag, mét teken
    const desc = row.slice(7).join(' '); // omschrijvingsveld (rest)

    out.push(
      makeRow(seen, {
        tx_date,
        amount,
        counterparty: extractTag(desc, 'NAME') || null,
        description: extractTag(desc, 'REMI') || clean(desc),
      }),
    );
  }

  return { rows: out, warnings, columns: ['(ABN AMRO TAB, kolom-vast)'] };
}

// Lees de waarde van één /TAG/ tot aan de volgende bekende tag.
function extractTag(desc, tag) {
  const marker = `/${tag}/`;
  const start = desc.indexOf(marker);
  if (start < 0) return null;
  const rest = desc.slice(start + marker.length);
  const next = rest.match(new RegExp(`/(?:${ABN_TAGS.join('|')})/`));
  const val = next ? rest.slice(0, next.index) : rest;
  return val.trim() || null;
}

// --- Gedeeld: rij + idempotente fingerprint -------------------------------

function makeRow(seen, { tx_date, amount, counterparty, description }) {
  ...
  return { tx_date, amount: rounded, counterparty, description, fingerprint: `${baseKey}#${occ}` };
}
