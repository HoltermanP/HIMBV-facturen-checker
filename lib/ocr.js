...
// dataUrl = "data:<contentType>;base64,<...>"; hint = optionele context; filename voor PDF-input.
export async function extractFields(dataUrl, hint, filename) {
  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey) return { ...EMPTY };

  try {
    // PDF's gaan als 'file'-input (image_url accepteert alleen afbeeldingen);
    // afbeeldingen blijven via image_url.
    ...
    const userContent = [
      {
        type: 'text',
        text: hint
          ? `Context: ${hint}. Lees onderstaand document en geef de gevraagde JSON.`
          : 'Lees onderstaand document en geef de gevraagde JSON.',
      },
      media,
    ];
