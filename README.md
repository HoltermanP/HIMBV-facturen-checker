...
1. Upload een **bankafschrift als CSV of TAB-bestand** (export uit je bank). De parser
   herkent automatisch het scheidingsteken (komma, puntkomma of **tab**), CSV's mét
   kopregel, én het **kolom-vaste ABN AMRO TAB-formaat** (zonder koppen, met `/NAME/` en
   `/REMI/` in het omschrijvingsveld). Her-uploaden geeft geen dubbele regels (idempotent
   op fingerprint).
